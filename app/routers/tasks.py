...
def create_task(task: schemas.TaskCreation, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def get_tasks( is_completed: Annotated[bool | None, Query()] = None, 
    ...
    if not sort_column:
        raise HTTPException( status_code= 400 , detail= "Invalid sort filed.Allowd value: id, title" )
    if order_by == "asc":
        query = query.order_by(sort_column.asc())
    elif order_by == "desc":
        query = query.order_by(sort_column.desc())
    else:
        raise HTTPException(status_code=400, detail="Invalid order value.Allowed values are asc, desc")
...
def partial_update(task_id: int, task_data:schemas.PartialUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
